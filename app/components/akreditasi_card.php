...
$colors = [
    ...
    'red'     => ['bg' => 'bg-red-50',      'text' => 'text-red-800',     'border' => 'border-red-900',     'btn' => 'hover:bg-red-800'],
...
<div class="group relative bg-white rounded-2xl shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col h-full card-animation" data-aos="fade-up">
    ...
    <div class="p-6 flex flex-col h-full">
        <div class="flex justify-between items-start mb-4">
            ...
            <?php else: ?>
                <svg class="w-6 h-6 text-red-950/20 group-hover:text-red-950 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            <?php endif; ?>
        </div>

        <div class="mb-4">
            <h3 class="text-2xl font-bold text-red-950 mb-1 leading-tight"><?= $item['sub'] ?></h3>
            <p class="text-sm font-semibold text-gray-500"><?= $item['prodi'] ?></p>
        </div>

        <div class="space-y-3 mb-6 grow">
            <div class="bg-gray-50 p-3 rounded-lg border border-gray-100 group-hover:border-red-100 transition-colors">
            ...
            <div class="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-tighter">
                <svg class="w-4 h-4 text-red-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                <?= $item['date'] ?>
            </div>
        </div>

        <a href="/unsoed_profile/public/assets/akreditasi/<?= $item['file'] ?>" target="_blank"
           class="w-full mt-auto inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-red-950 text-white text-sm font-bold shadow-lg shadow-red-900/20 transition-all duration-300 <?= $c['btn'] ?> group-hover:shadow-red-950/30">
